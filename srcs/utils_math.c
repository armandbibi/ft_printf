...
/*   Updated: 2018/05/15 15:40:42 by abiestro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (PTF_LEN_Z(arg->l_modifier))
		ft_unsigned_itoa((uintmax_t)value, tmp, base, 10); 
	else if (PTF_LEN_LL(arg->l_modifier))
		ft_unsigned_itoa((unsigned long long)value, tmp, base, 10); 

	else if ((PTF_LEN_L(arg->l_modifier) || arg->type == 'D'))
		ft_unsigned_itoa((unsigned long)value, tmp, base, 10);
	else if (PTF_LEN_J(arg->l_modifier) && (arg->is_negative = 0))
